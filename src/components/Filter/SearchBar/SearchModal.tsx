  ...
  ({ query, onClose, onSelect }, ref) => {
    ...
    const handleSelect = (value: string) => {
      ...
      if (existingFiltersParam) {
        try {
          existingFilters = JSON.parse(
            decodeURIComponent(existingFiltersParam)
          );
      ...
      switch (item.label) {
        case "metro":
          ...
          break;
        case "districts":
          if (!filters.districts.includes(value))
            filters.districts.push(uaValue);
          break;
        case "street":
